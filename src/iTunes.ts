...
export default class iTunes {
      ...
      .then((result) => {
        if (result == null) {
          window.showErrorMessage(
            "Visual Studio Code hasn't access to Accessibilty of your macOS. Please enable at System Preferences -> Security & Privacy"
          );
        }
      })
      .catch(() => {
  ...
  private getScript(
    ...
    const file = path.resolve(
      __dirname,
      `../../scripts/${app}/${language}/${filename}.applescript`
    );
    ...
    return path.resolve(
      __dirname,
      `../../scripts/${app}/en/${filename}.applescript`
    );
  }
